...
// Resize equivalent of computeSnap — the corner being dragged moves exactly
// one horizontal edge (left or right) and one vertical edge (top or
// bottom); the opposite edges stay anchored. Snaps whichever edge is moving
// to a neighbor's edge (or the grid), so resizing e.g. an image to line up
// with the one next to it locks into place instead of stopping a pixel off.
function computeResizeSnap(
  corner: "se" | "sw" | "ne" | "nw",
  x: number, y: number, ow: number, oh: number,
  candidates: { xs: number[]; ys: number[] } | null,
  zoom: number, thresholdPx: number, ruling: Ruling, gridStep: number,
): { x: number; y: number; ow: number; oh: number; guideX: number | null; guideY: number | null } {
  const thresholdWorld = thresholdPx / zoom;
  const movesRight  = corner === "se" || corner === "ne";
  const movesBottom = corner === "se" || corner === "sw";

  let nx = x, nw = ow;
  const curXEdge = movesRight ? x + ow : x;
  let bestXDist = thresholdWorld, snapX: number | null = null;
  if (candidates) for (const cx of candidates.xs) {
    const d = Math.abs(curXEdge - cx);
    if (d < bestXDist) { bestXDist = d; snapX = cx; }
  }
  if (snapX === null && ruling === "grid") {
    const nearest = Math.round(curXEdge / gridStep) * gridStep;
    if (Math.abs(nearest - curXEdge) < thresholdWorld) snapX = nearest;
  }
  if (snapX !== null) {
    if (movesRight) nw = snapX - x;
    else { nw = ow + (x - snapX); nx = snapX; }
  }

  let ny = y, noh = oh;
  const curYEdge = movesBottom ? y + oh : y;
  let bestYDist = thresholdWorld, snapY: number | null = null;
  if (candidates) for (const cy of candidates.ys) {
    const d = Math.abs(curYEdge - cy);
    if (d < bestYDist) { bestYDist = d; snapY = cy; }
  }
  if (snapY === null && ruling === "grid") {
    const nearest = Math.round(curYEdge / gridStep) * gridStep;
    if (Math.abs(nearest - curYEdge) < thresholdWorld) snapY = nearest;
  }
  if (snapY !== null) {
    if (movesBottom) noh = snapY - y;
    else { noh = oh + (y - snapY); ny = snapY; }
  }

  return { x: nx, y: ny, ow: nw, oh: noh, guideX: snapX, guideY: snapY };
}
...
function WhiteboardCanvas({ roomId, role = "student", materials = [], myName }, ref) {
  ...
  const myColor = role === "tutor" ? "#5555e0" : PARTICIPANT_COLORS[parseInt(mySenderIdRef.current.slice(0,6), 36) % PARTICIPANT_COLORS.length];
  // Follow-me / presenter mode: tutor toggles `presenting`, student tracks who (if
  // anyone) they're currently following. Opting out ("Отвязаться") just clears it;
  // a fresh follow_state:true broadcast re-engages regardless of a prior opt-out.
  ...
  // Jump-to-participant, follow_state, and continuous follow-mode mirroring all
  // fly through here — 400-600ms ease-in-out by default; follow-mode's continuous
  // updates pass a shorter duration since those arrive every ~120ms.
    ...
    let ch = supabase
      ...
      .on("broadcast", { event: "draw" }, ({ payload }: { payload: WsEvent }) => {
        ...
        if (payload.type === "viewport")  {
          ...
          // Follow-me: only mirror the current leader's viewport, and only while
          // actively following (not after opting out). Animated, not an instant snap.
  ...
  // Tutor-only: toggle presenter mode. Turning it on flies every student to the
  // tutor's current view, then keeps mirroring the tutor's pan/zoom live until
  // toggled off (or a student opts out individually via "Отвязаться").
  const togglePresent = () => {
  ...
  const broadcastCursor = (wx: number, wy: number) => {
    ...
    const name = myName ?? (role === "tutor" ? "Репетитор" : "Ученик");
    send({ type: "cursor", x: wx, y: wy, name, senderId: mySenderIdRef.current, color: myColor });
  };
    ...
    if (selDragRef.current) {
      ...
      if (drag.mode === "move") {
        ...
        if (e.altKey) {
          ...
          const gridStep = rulingSizeRef.current === "S" ? 20 : rulingSizeRef.current === "L" ? 60 : 36;
          const snap = computeResizeSnap(drag.corner, x, y, ow, oh, snapCandidatesRef.current, viewRef.current.zoom, SNAP_THRESHOLD_PX, rulingRef.current, gridStep);
          x = snap.x; y = snap.y; ow = Math.max(minSize, snap.ow); oh = Math.max(minSize, snap.oh);
          snapGuidesRef.current = { x: snap.guideX, y: snap.guideY };
        }
    ...
    if (selDragRef.current) {
      ...
      if (idx >= 0) {
        if (drag.mode === "move") {
          ...
          const gridStep = rulingSizeRef.current === "S" ? 20 : rulingSizeRef.current === "L" ? 60 : 36;
          const snap = computeResizeSnap(drag.corner, x, y, ow, oh, snapCandidatesRef.current, viewRef.current.zoom, SNAP_THRESHOLD_PX, rulingRef.current, gridStep);
          x = snap.x; y = snap.y; ow = Math.max(20, snap.ow); oh = Math.max(20, snap.oh);
          snapGuidesRef.current = { x: snap.guideX, y: snap.guideY };
          itemsRef.current[idx] = { ...orig, x, y, w: ow, h: oh };
  ...
  const renderMinimap = useCallback(() => {
    ...
    // Student viewport indicator — tutor only, tinted with the student's own color
    if (role === "tutor") {
    ...
    <div className="flex flex-1 overflow-hidden select-none" style={{ touchAction: "none" }}>
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="hidden sm:flex items-center border-b shrink-0"
          ...
          {/* Left: tool-specific options — text formatting takes over this
              whole strip while editing text (see below); everything else
              only shows when that's not the case. */}
          {!textInput && <>
          ...
          {/* Text formatting — docked in this fixed header strip instead of
              floating next to the text box on the canvas. A floating panel
              had no way to know what was printed on the page underneath —
              on a dense worksheet (short answers in small cells) it kept
              landing on top of a neighboring line no matter how small it
              was shrunk. This strip is always in the same reserved spot
              above the board, so editing never covers board content. */}
          {textInput && !isMobile && (() => {
            const iconBtn = "w-7 h-7 rounded-lg flex items-center justify-center transition-colors shrink-0 hover:bg-black/5";
            return (
              <div className="flex items-center gap-0.5 flex-wrap">
                <select value={fontIdx} onChange={e=>setFontIdx(+e.target.value)} onMouseDown={e=>e.stopPropagation()}
                  title="Шрифт" className="border-0 rounded-lg outline-none cursor-pointer"
                  style={{ color:"var(--brown-dark)", height:28, maxWidth:100, fontSize:12, background:"transparent", fontFamily: FONTS[fontIdx].family }}>
                  {FONTS.map((f,i)=><option key={i} value={i} style={{ fontFamily:f.family, fontSize:15 }}>{f.label}</option>)}
                </select>
                <Sep/>
                <button onMouseDown={e=>e.preventDefault()} onClick={()=>setFontSize(s=>Math.max(8,s-2))} title="Меньше" className={iconBtn} style={{ color:"var(--brown-dark)" }}>
                  <span className="text-xs font-bold">A−</span>
                </button>
                <span className="text-xs tabular-nums text-center shrink-0" style={{ width:22, color:"var(--brown-mid)" }}>{fontSize}</span>
                <button onMouseDown={e=>e.preventDefault()} onClick={()=>setFontSize(s=>Math.min(200,s+2))} title="Больше" className={iconBtn} style={{ color:"var(--brown-dark)" }}>
                  <span className="text-xs font-bold">A+</span>
                </button>
                <Sep/>
                ...
                <label className={`relative ${iconBtn} cursor-pointer`} title="Цвет текста">
                  <span className="font-bold leading-none" style={{ color:"var(--brown-dark)", fontSize:12 }}>A</span>
                  <div className="absolute rounded-full" style={{ bottom:5, left:8, width:12, height:2.5, background:color }}/>
                ...
                <Sep/>
                <button onMouseDown={e=>e.preventDefault()} onClick={commitText} title="Готово (или клик вне текста)"
                  className="flex items-center gap-1 px-2.5 h-7 rounded-lg text-xs font-semibold text-white shrink-0"
            ...
            {role==="tutor" && (
              <>
                <button onClick={togglePresent}
                  title={presenting ? "Остановить показ — ученик снова свободен" : "Показать всем / Привести ко мне — ученик следует за вашим экраном"}
                  className="p-1.5 rounded-lg border-2 transition-all"
          ...
          return (
            <GrammarExerciseOverlay key={gi.id} item={gi} sp={sp} sw={sw} sh={sh} selected={sel}
              ...
              onResizeStart={(corner, clientX, clientY) => {
                ...
                snapCandidatesRef.current = collectSnapCandidates(itemsRef.current, new Set([gi.id]));
              }} />
          );
        })}
        ...
        {/* Snap alignment guides — shown while dragging an item/selection
            ...
            Figma's guides do. */}
        {(selDragRef.current?.mode === "move" || selDragRef.current?.mode === "resize-img" || selDragRef.current?.mode === "resize-frame" || multiDragRef.current) && snapGuidesRef.current.x !== null && (() => {
        ...
        {(selDragRef.current?.mode === "move" || selDragRef.current?.mode === "resize-img" || selDragRef.current?.mode === "resize-frame" || multiDragRef.current) && snapGuidesRef.current.y !== null && (() => {
                    ...
                    return (
                      <div key={corner} className="absolute pointer-events-auto"
                        ...
                        onMouseDown={e => {
                          ...
                          snapCandidatesRef.current = collectSnapCandidates(itemsRef.current, new Set([selectedItem.id]));
                        }}
                        onTouchStart={e => {
                          e.stopPropagation(); e.preventDefault();
                          const rect = containerRef.current!.getBoundingClientRect();
        ...
        {/* Text input — textarea inline on the board; formatting controls
            live in the fixed header strip now (see the context bar), not
            floating next to this. */}
        {textInput && textScr && (() => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderRadius:8, border:`1px solid ${fromHidden ? "rgba(124,92,62,0.25)" : "rgba(74,128,240,0.3)"}`,
          display:"flex", alignItems:"center", justifyContent:"center", padding:"6px 8px",
        }}>
          <span style={{ fontSize:"0.75rem", fontWeight:700, color: fromHidden ? "#3b2a1a" : "#1a3c8a", textAlign:"center" }}>
        ...
        <div style={{
          ...
          borderRadius:8, border:`1px solid ${fromHidden ? "rgba(74,128,240,0.3)" : "rgba(124,92,62,0.25)"}`,
          display:"flex", alignItems:"center", justifyContent:"center", padding:"6px 8px",
        }}>
          <span style={{ fontSize:"0.75rem", fontWeight:700, color: fromHidden ? "#1a3c8a" : "#3b2a1a", textAlign:"center" }}>
